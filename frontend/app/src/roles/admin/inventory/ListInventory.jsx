...
export default function ListInventory() {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://127.0.0.1:8000/delete/inventory/${id}`);
      alert("Inventario eliminado exitosamente");
      fetchInventories();
    } catch (error) {
      console.error("Error deleting inventory:", error);
      alert("Error eliminando inventario");
    }
  };
                  ...
                  {inventories.map((inventory) => (
                    <TableRow key={inventory.inventario_id}>
                      ...
                      <TableCell>
                        ...
                        <Button variant="contained" color="secondary" onClick={() => handleDelete(inventory.inventario_id)}>
                          Eliminar
                        </Button>
                      </TableCell>
                    </TableRow>
